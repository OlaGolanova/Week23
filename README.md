### Вопросы 💎 Week29

### Вопросы 💎

1. Можно ли создать несколько контекстов в одном приложении?

Да. Можно


2. Можем ли мы влиять на контекст из вложенных компонентов?
да. С помощью функции  createContext() создается контекст, затем создается провайдер компонента Provider, чтобы предоставить значение контекста для всех своих потомков. 
Вложенные компоненты могут получить доступ к значению контекста, используя функцию useContext().


3. Как выбрать компонент, в котором будет определяться контекст?

Это должен быть компонент, в котором мы хотим получит данные от провайдера. 


4. Возможно ли создать контекст, который доступен только в части приложения? 
Да, возможно. Его для этого и используют.


5. На каком этапе жизненного цикла компонента лучше всего запросить данные с сервера?

После монтирования компонента в DOM, сразу после рендеринга,  метод componentDidMount. При первой загрузке  useEffect(() => {  }, []);

```jsx

useEffect(() =>{                     //Данный хук аналогичен методу componentDidMount
        fetch('/api/words')
            .then((response) => response.json())
            .then((response) => setWords(words = response))
    }, []); 
```



6. Вернёт ли сообщение об ошибке Fetch API, если код ответа 401 (ошибка авторизации)?
Да, вернется ошибка авторизации на сервере.


7. Каким ещё способом можно делать запросы к API кроме fetch?
XMLHttprequest, graphql, Axios


8. Зачем нужен CORS?
Для нашей безопасности, браузер предотвращает возможность стороннего источника подключится к нашему браузеру.

Чтобы обеспечить безопасность запросов к другим источникам, браузеры используют механизм под названием CORS. 
CORS работает по принципу единого источника. Мы можем получить доступ к ресурсам только в том случае, если источник этих ресурсов и источник запроса совпадают.
Идет сравнение по  протоколу, домену или порту.

CORS (Cross-Origin Resource Sharing, англ.  «совместное использование ресурсов разных источников») — это стандарт, позволяющий предоставлять веб-страницам доступ к объектам сторонних интернет-ресурсов.
Сторонним считается любой интернет-ресурс, который отличается от запрашиваемого протоколом, доменом или портом.



### Вопросы 💎 Week28


 # ПРАКТИЧЕСКОЕ ЗАДАНИЕ #1: в компоненте SpamFilter.

  https://olagolanova.github.io/Week23/

# Вопросы:

1. В чём разница между контролируемыми и неконтролируемыми компонентами?
У управляемых компонентов есть атрибут их текущего состояния и метод, вызывающийся при их изменении.  Их данные хранятся в состоянии React. Неуправляемые компоненты - это компоненты, значение которых читается из DOM и зменение таких компонентов не отселживется.

2. Есть ли смысл использовать метод `shouldComponentUpdate()` в `PureComponent`?
Нет. При использовании `PureComponent` можно не добавлять метод жизненных циклов shouldComponentUpdate к компонентам: отслеживание изменений происходит само собой, 
при этом проводится неглубокая проверка. 

3. Будет ли перерисовываться данный компонент?   
    
    ```jsx
    class PureComponent extends React.PureComponent {
      state = { 
        item: {
    			count:0
    		},
      }
      
      handleClick= () =>  {
        const item = this.state.item;
        item.count = this.state.item.count ++ ;
        this.setState({item});
      }
      render() {
        return <h2>{this.state.item.count}</h2>
      }
    }
    ```
Нет, потому что  item это объект {}, при изменении объекта он будет мутировать. А так как объект ссылочный тип данных, то ссылка будет на ту же ячеку памяти, PureComponent проводит не глубокую проверку - это значит, что для него ничего не поменяется, компонент отрисован не будет.
    
4. Что будет, если чекбоксу не передать свойство '`checked`'?
Он будет отрисовываться как не выбранный.

5. В чём главное преимущество использования `PureComponent`?
PureComponent будет вызывать рендер, только если обнаружит изменения в state или propsю.  Таким образом, сокращается количество рендеров в приложении, а значит и React производит намного меньше операций. Это и даёт плюс производительности. При условии, что компонент является «Pure», в React проводится неглубокая проверка.

6. Что будет, если компоненту `input` передать метод `onChange`, но не передать `value`? А что будет, если компоненту `input` передать `value`, но не передать метод `onChange`?
Если передать input, но не передать метод `onChange` - то можно будет считать из инпута только единственое значение value. Если передать `onChange`, но не передать value, то будет отслеживаться каждый вводимый символ в инпут

7. Как сделать из обычного `select` список с несколькими выбранными значениями (мультиселект)?

В атрибут value можно передать массив, что позволит выбрать несколько опций в теге select .
```jsx
<select multiple={true} value={['Б', 'В']}>
```

8. Напишите пример валидации текстового поля на React, чтобы оно было не пустым.

Решение в компоненте App.js

 https://olagolanova.github.io/Week23/

9. Приведите пример простейшей формы логина на сторонних компонентах (Formic, Material или Bootstrap на ваш выбор).
Решение в компоненте App.js


 https://olagolanova.github.io/Week23/




 



### Вопросы 💎  Week27

1. Как вы думаете, чем плохо задание атрибута компонента `key` функцией Math.random()?

Функция Math.random() мржет повторять числа, это значит будет немколько компонентов с одним key, возникнут сложности с редактированием или удалением такого компонента.

2. Как будет выглядеть [этот](https://www.notion.so/27-ea715ee9a7414d88916c15cec7a71515?pvs=21) пример, если мы кроме id будем передавать ещё один параметр title?

Возможно так:

```jsx


  onClick = (id) => (e) => (title) => {
        e.preventDefault();
        this.setState({ count: this.state.count + 1 });
    };

  render() {
    const {title} = this.props;
      const {id} = this.props;
      const {count} = this.state;
      return (  
            {title}
            <a href="#" onClick={this.onClick(id)}>click {count}</a>);
            
  }

```


3. В чём отличие VirtualDOM от обычного DOM?
DOM  — способ представления структурного документа с помощью объектов. DOM изначально не был предназначен для динамического изменеия объектов. 
В React, мы не взаимодействовуем с DOM напрямую, мы работаем с его легковесной копией VirtualDOM. Вносим изменения в копию, исходя из наших потребностей, а после этого применяя эффективные алгоритмы сравнения, изменеется реальный DOM. При этом происходит сравнение DOM-дерева с его виртуальной копией, определяется разница и запускается перерисовка того, что было изменено.

4. В каком порядке выйдут сообщения в консоли и почему?
    
    ```jsx
    handleChange = () => {
    	console.log('foo');
    
    	this.setState({
    		checked: !this.state.checked, 
    		},
    		()=>{console.log('baz')}
    	);
    	console.log('bar') 
    };
    ```

    foo, bar, а затем baz, так как она в колбеке. Колбек вызовится после успешного обновления состояния компонента
    
5. Какую проблему решает использование рефов?
Рефы позволяют получать доступ к элементам DOM, созданным компонентами, и изменять их свойства и методы непосредственно, а не через виртуальный DOM???

6. Как вы думаете, почему вызов методов ребёнка из родительского компонента противоречит философии реакта?
В реакте поток данных — однонаправленный. Это значит, что данные передаются сверху вниз, от родителя к ребенку, через props. При этом props — неизменяемый объект, предназначенный только для чтения. Если мы начинаем вызывать методы дочернего компонента из родительского, мы нарушаем эту однонаправленность и создаем ситуацию, когда возможны неожиданные и трудно отслеживаемые изменения состояния компонента.

Вместо вызова методов дочернего компонента можно использовать передачу пропсов, чтобы родительский компонент оповещал дочерний о необходимости выполнить определенное действие. Таким образом, мы сохраняем однонаправленный поток данных и сохраняем четкую структуру компонентов, что способствует легкости поддержки и понимания кода.

7. Можно ли с помощью хука `useRef` передать `ref` дочерним элементам? 
Да, можно.

8. Что даёт нам использование кастомных хуков?

Кастомные хуки нужны для того, чтобы записать определенную логику, которой нет в стандартных хуках в функцию и в дальнейшем переиспользовать. Внутри них испрльзуют стандартные хуки React, такие как useState, useEffect и т.д.



### Вопросы 💎 Week26

1. Если в [этом](https://www.notion.so/26-f67d3134cbf54844831a5d899db00b36?pvs=21) примере изменить порядок Route таким образом:
    
```jsx

    <Routes>
    	<Route path="/" element={<Home />} />
    	<Route path="/about" element={<About/>} />
    	<Route path="/users" element={<Users />} />
    </Routes>

```
    
    то какой компонент будет отрисован по адресу /users? Объясните почему.

    Будет отрисован компонент <Users/>. В версии реакт роутера 6 не имеет значение в каком порядке прописаны пути.
    
2. Какое сообщение появится на экране по адресу http://localhost:3000/users/12345

```jsx
...
<Route path='/users/:number' element={<User />} />
...
    
function User(props) {
    const number = props.match.params.number;
    return(
        number===12345
            ? <h1>Моя личная страница</h1>
            : <h1>Страница пользователя {number}</h1>
    );
}
```
       Моя личная страница 
    
3. Вспомните, какой второй параметр принимает метод `map` .
   index — индекс текущего элемента; 


4. Как бы вы подошли к решению задачи по выводу компонентом `<CardList>` только тех экземпляров компонента `<Card>`, цена которых не превышает заданную?

```jsx
{
    id: 111345,
    title:"Зимние ботинки",
    price:99,
    description:"Такие только у нас",
    imgLink:"https://shop/winter_boots.jpg"
}



function CardList() {
        return (
            <React.Fragment>
                {shoes.filter((item, index)=> {
                        if (item.price < 100) {
                        <Card
                            key={index}
                            id={item.id}
                            title={item.title}
                            price={item.price}
                            description={item.description}
                            imgLink={item.imgLink}
			            />
                 } })}
            </React.Fragment>
        );
}
```


5. Как задать параметр в пути? Например, *filter (*`useLocation, useSearchParams`*).*

```jsx

<Link to={`/Words/training/${filter}`}>

```

6. Какая разница между `element` и `children` в указании роутера?
Element обозначает конкретный элемент (компонент), который должен быть отображен при совпадении пути с определенным маршрутом. В этом случае, только один элемент будет отображен на странице при совпадении с путем.
 
Children представляет собой группу элементов (компонентов), которые могут быть отображены одновременно при совпадении пути с определенным маршрутом. В этом случае, все дочерние элементы указанного маршрута будут отображены на странице при совпадении с путем.



7. Зачем нужен `exact`?

В реакт роутер ранних версий exact необходим для указания точного сопадения пути.
В реакт роутер 6 нет exact. Благодаря звездочке в нем пропала необходимость. Если звездочка в пути отражает, что роут может матчиться с вложенными роутами. Если же звездочки нет — то только при точном совпадении пути. 


8. Самостоятельно разберитесь, зачем нужны `useMatch`, `useLocation`, `useNavigate`? 

Хук useLocation возвращает объект location , представляющий текущий URL.

Хук возвращает функцию, которая в качестве первого аргумента принимает строку URL или целое число. 
Целое число может быть положительным (движение вперед по истории браузера) или отрицательным (движение назад по истории браузера). 
В качестве второго аргумента можно передать объект {replace, state}.

Хук useMatch необходим для того чтобы сравнить соответвует текущий url указанному пути в переданном аргументе.


9. Как можно сделать перенаправление на другую страницу по клику на кнопку с помощью `useNavigate`? *Ищите ответ в документации к react-router-dom.*

  <button onClick={() => navigate('/target-page')> -в скобках нужно указать адрес страницы куда будет переход, либо число, если число отрицательное, то будет переход назад по истории браузера, если положительное-вперед по истории. В качестве второго аргумениа можно передать {replace, state} для замены страницы на которую переходим при клике на другую.


### Вопросы 💎 Week25

Практическое ДЗ
Попробуйте переписать классовый компонент Counter, используя хуки:

```jsx


import React, { useState } from 'react';



function Counter2() {
    const [count, setCount] = useState(0);
    
    
    function handleClick(){
        setCount ( count + 1 );
    }
    return (
        <>
            <button onClick={ handleClick }>{ count }</button>
        </>
    );
}
  
export default Counter2;

```

1. В компонент передаются атрибуты `description` и `title` . Могу ли я их сложить как на примере, чтобы получить одну строку и вывести в компоненте?
    
    ```jsx
    import React from "react";
    import styles from './button.css'
    
    export default class Example extends React.Component {
        render() {
    				let {description,title} = this.props
    				title += description; //title = title + description
             return (
                <span>{title}</span>
            );
        }
    }
    ```

    Нет, пропсы надо менять через this, this.state.title = title + description;

    
2. С помощью какого метода можно отловить изменение `props`?
Можно отловить изменеие пропсов с помощью добавления второго аргумента в useEffect.

Метод shouldComponentUpdate(nextProps, nextState): вызывается каждый раз при обновлении объекта props или state. В качестве параметра передаются новый объект props и state. Эта функция должна возвращать true (надо делать обновление) или false (игнорировать обновление). По умолчанию возвращается true.



3. Оператор расширения часто используется также для клонирования объекта. Подумайте, чем отличаются эти две записи и какую проблему решает здесь оператор расширения:
    
    ```jsx
    const initialObj = { title:'Hello', text:'World' }
    
    //№1
    const firstObj = initialObj  //Полность копирует объект,  firstObj зависит от initialObj, firstObj будет менятся, если изменится initialObj
    
    //№2
    const secondObj = {...initialObj} //secondObj может отличаться по содержанию от initialObj, независимые объекты
    ```
    
4. В каком из методов жизненного цикла лучше всего использовать методы вызова API и обращения к веб-хранилищам, если они должны быть вызваны всего один раз при загрузке страницы?

componentDidMount() для классового компонента, useEffect для функционального

5. С помощью какого метода можно отловить и отрисовать для пользователя возникшую в компоненте ошибку?
Для отлова и отображения ошибок можно использовать метод жизненного цикла componentDidCatch(). Создается компонент ErrorBoundary, который оборачивает другие компоненты. Если в дочернем компоненте происходит ошибка, метод componentDidCatch() будет вызван, и компонент ErrorBoundary отобразит сообщение об ошибке для пользователя. 

6. Какой код обычно пишут в конструкторе? Для каких задач он используется?
Внутри конструктора происходит инициализация компонента, устанавливается состояние (state) и пропсы (props). А такеж привязка обработчиков событий к экземпляру.

  ```jsx
   
export default class Example extends React.Component {
    constructor(props) {
    
        super(props);
        this.state = {
            date : new Date(),
            clickedStatus: false
        };
    }
}
    ```

7. Что делает функция `render`()? Что может её вызвать?
render вставляет компонент в DOM, отрисовывает его на странице или перерисовывает после изменений.
render вызывается после инициализации компонента и после изменения компонента-повторный рендер.

8. Что нужно изменить в коде из урока (видео), чтобы начальные параметры у компонента приходили из пропсов, но если пропсы вообще не заданы, начальные значения были инициализированы нулями?

```jsx
```

  ```jsx
   
export default class Example extends React.Component {

    constructor(props) {
    
       super(props);
            this.state = {
                min: 0,
                sec: 0
            };
    }
    render()
        return(
            <div>
                Timer info: { this.state.min ? this.state.min : 0  } : { this.state.sec ? this.state.sec : 0 }
            </div>
        )
    
}
    ```



9. Можно ли несколько раз использовать хук `useEffect` внутри одного компонента?
Да. useEffect может быть много.

10. Можно ли не передавать второй аргумент в хук `useEffect`? Что тогда произойдёт?
Да. Если передать вторым арргументом примитив, то useEffect и функция внутри него сработает в случае изменения состояния этого примитива, но такой способ не сработает для объекта или имассива. В этом случае надо передать пустой массив, и тогда useEffect сработает при добавление или удалении кмпонента.

11. Что означает возвращение функции в теле хука `useEffect`? 
в return возвращается колбек функция, которая может отменить действие, которое сделал этот useEffect и которое в дальнейшем уже не нужно выполнять, например отменить подписку API, удалить слушателя JS,

12. Будут ли перерисованы дочерние элементы компонента при вызове метода `forceUpdate`()?
Да.







### Вопросы 💎 # Week24

1. Кратко опишите основные отличия `state` от `props`. 
В `props` передаются данные о состоянии компонента, которые нельзя менять, данные переадются извне.
В`state` хранитятся  данные о компоненте, которые можно менять, они полностью зависят от компонента и доступны из компонента.



2. Какую ошибку я допустила в следующем коде и как её исправить: 
    
    ```jsx
    handleChange = () => {
        const chechedArr = this.state.arr; // [1,2,3] 
    		chechedArr.push(5); // [1,2,3,5] 
        this.setState({
          arr: chechedArr,
        });
      };
    ```
    Я думаю ошибка в том, что state необходимо задать внутри constructor перед  функцией handleChange;

 ```jsx

    constructor(props) {
        super(props);
        this.state = {
            const chechedArr = this.state.arr; // [1,2,3]
        };
    };
    handleChange = () => {
    	chechedArr.push(5); // [1,2,3,5] 
        this.setState({
          arr: chechedArr,
        });
      };
    ```

3. Можно ли повесить на один элемент несколько обработчиков событий?
Да

4. Каким образом можно изменить `state`?
С помощью функции setState для классовых компонентов и с помощью хука useState для функциональных компонентов

5. Попробуйте предсказать, какие сообщения будут в консоли, и объяснить результат:
    
    ```jsx

    handleChange = () => {
        console.log(this.state.checked) //#1 false 
        this.setState({
          checked: !this.state.checked, //true
        });
        console.log(this.state.checked) //#2 ?   Тут будет false, также как в первом случае, 
                                                так как setState асинхронная функция и будет выполнятся после console.log #1 и #2
      };  

    ```
    
6. Какими способами можно задать функцию `handleChange` и какой из них является самым правильным?
Функию можно задать как стрелочную или как обычную, правильно задавать как стрелочную:

```jsx
    handleChange = () => {
            /*** Меняем состояние checked на обратное ***/
        this.setState({
        checked: !this.state.checked,
        });
    };
```

```jsx
    const handleChange = () => {
            setChecked(!checked);
        };

```

7. Чем отличаются классовые и функциональные компоненты?
У функционального компонента отсутствует render(), есть только return(). Не бывает функции конструктора и this, как у классового компонента.
Функциональные компоненты React удобные и современные, занимают меньше места при написании. Но классовые компоненты эквивалентны функциональным по своим свойствам, все можно написать на классовых или функциональных компонентах;

8. Есть ли `this` в функциональных компонентах? Как можно получить доступ к нему?
В функциональных компонентах не используем this. Нет необходимости, так как используем хуки.

9. Можно ли использовать `props` и `state` одновременно?
Да

10. Где можно задать `state` без использования команды `this.setState`?

Состояние можно передавать в виде свойств от родителя к дочернему компоненту.

Добавить значение state по умолчанию через defaultProps?????

11. Может ли состояние классового компонента не быть объектом? А функционального?
В классовых компонентах state-всегда объект. В функциональных состояние может, но не обязано, быть объектом. Исходное значение аргумента используется только при первом рендере.

12. Способны ли функциональные компоненты самостоятельно хранить состояние?
Нет, в функциональных компонентах состояние хранится в хуке useState.

13. Как использовать хуки в классовых компонентах?
Нельзя использовать хуки в классовых компонентах.
Хуки не работают внутри классов, а используются вместо них.

14. А как задать начальное состояние `props`, если они ещё не были переданы? 
С помощью defaultProps : значение props name задали по умолчанию

    ```jsx
            static defaultProps = {
                name: 'Hello!',
            };
    ```












# Week23
1. Какую ошибку я допустила в [этом](https://www.notion.so/23-CSS-React-d4b0b61bb697459aacb1681ba2764440?pvs=21) примере?  

Стоит лишний пробел между 1.5 и em 
	fontSize: '1.5 em',
    должно быть 	fontSize: '1.5em'

2. Какие есть способы работы со стилями в React?

Компоненты в React можно стилизовать четырьмя способами: инлайновые стили(нужно избегать), таблицы стилей, CSS модули и препроцессоры. еще есть разные дополнительные плагины и расширения, например styled compoinents.

3. Как будет выглядеть карточка товара, если ей передать атрибут `addedToCart`===0?

У карточки не будет кнопки Добавить в корзину, будет форма, в которой будет указано, что количетсво данного товара в корзине - 0.

4. Какие ещё проверки нужно было бы сделать для атрибута `addedToCart`?

Проверка на ноль, если в корзине ноль товаров или меньше нуля(не натуральное число), то возвращаем кнопку добавить, форма с количеством товара уходит.
Проверка на максимально возможное количество товаров, чтобы не было возможности добавить товара больше, чем есть.

5. Клиент попросил повесить тег «New» на товары из новой коллекции. Как это сделать, какой условный оператор выбрать?

Условный оператор &&

6. Какими тремя способами можно написать условный рендеринг? 
Через If, тернарный оператор, &&

7. Представьте, что вы пишете компонент логина. Если пользователь авторизован, то мы показываем его имя, а если нет, то даём возможность ввода логина и пароля. Какой код для этого нужно написать, если за авторизацию пользователя отвечает флаг isAutorized?

Компонент формы авторизации
  ```jsx
           export default function AddToCartForm() {
                return (
                    <form>
                        <input placeholder="Введитие логин"/>
                        <input placeholder="Введитие пароль"/>
                        <button>Вход</button>
                    </form>
                );
            }
```


Компонент Приветсвия с Имененм
```jsx

            export default function AddToCartButton(props) {
                return (
                    <div>
                        Привет, {props.name}
                    </div>
            
                )
            }
                                 
  ``` 


  
Главный компонент, принимающий разные состояния
  ```jsx
            import React from 'react';

            import AddFormAuthorization from './AddFormAuthorization';
            import HelloUser from './HelloUser';



            export default function Logo(props) {
                const {nameUser="Оля", isAutorized } = props;
                
                return (
                    <div >
                            {isAutorized 
                                ? <HelloUser name={nameUser} />
                                :  <AddFormAuthorization />
                            }
                    </div>
                );
            }
```


8. В чём преимущества использования препроцессоров? Какой ещё есть способ использовать переменные, кроме $ в препроцессорах?
Можно задавать переменные, миксины, использовать математические операции, вложенность классов, амперсанд, чтобы не повторяться в написании классов.
